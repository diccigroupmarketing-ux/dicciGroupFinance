...
- SAMBUNG SESI BARU (owner nyatakan 25 Jul, kerja sesi ni SEMUA committed + pushed + deployed, working tree bersih di dbfcacb). Keutamaan pending:
  - (A) KEPUTUSAN OWNER pending: item 7 "amountGuard" dari audit /timbang, duit rosak jadi RM0 senyap sebab db.to_num guna pd.to_numeric(errors=coerce).fillna(0). SENGAJA ditangguh sebab db.to_num dikongsi recon (risiko parity). Owner kena putus sama ada nak kuarantin nilai rosak (bukan RM0) atau biar. Bukan bug tercetus, risiko laten.
