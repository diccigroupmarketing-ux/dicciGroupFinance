...
## Status
- progress: (2 Ogos: FOUNDATION FLEET SIAP + PUSH + CI PERTAMA HIJAU) 9 kerja foundation ikut tangga dalam 6 commit (1ef7ba4..d746c33, SEMUA dah push, run CI GitHub pertama LULUS penuh). TANGGA 2 penyatuan enjin: Pilihan 1 diratifikasi owner, recon.ts = enjin produksi KANONIK, reconcile.py = ORACLE beku (jaga baseline, bukan runtime app), reconSql.py = BEKU bersara bersama Streamlit tangga 5 (penemuan: E1 bukan enjin runtime mana mana app, app.py guna E2, webApp guna E3, jadi pendua sebenar cuma E2); fasa 0 tanda peranan dalam kod + docs, fasa 1 guard konstan, fasa 2 konstan recon.ts DIJANA dari Python (genReconConstants prebuild, reconConstants.ts di-commit, literal tangan dibuang, mustahil lari senyap), fasa 3 fixture adversarial AWB kongsi + whitespace; fasa 4 (padam E2) tunggu tangga 5, fasa 5 (SQL view) tunggu tangga 6-7. PARITY 3 ENJIN kini langkah AKHIR npm test (suite 22 langkah, RECON_TODAY dipaksa, gagal kuat bila fixture tiada) + fixture kunci gap sen digit ke-3 sisi Postgres dan tarikh tepi kalendar (31/30, 29 Feb, NULL aging). CI PERTAMA REPO (.github/workflows/ci.yml + scripts/testCi.mjs): 15 langkah subset 100% sintetik setiap push, 6 kekal skip perlu data sebenar, lejar jujur "hijau CI bukan suite penuh" (docs/ciCoverage.md), termasuk SEGI TIGA PARITY E1/E2/E3 atas fixture emas sintetik 15 kategori (genGoldenFixture on-the-fly + testGoldenParity E1 lwn E2 + seedGoldenPg untuk parityCheck E2 lwn E3). RATCHET query mentah (checkRawQueryRatchet, baseline 203 titik dalam 14 fail lib, injap sehala; penemuan: bait NUL sengaja dalam dashboardRange.ts buat grep undercount 4 titik, audit berasas grep tak boleh dipercayai di situ). INVENTORI company_id tangga 3 siap dalam vault (16 jadual tenant + 2 global, semua titik query terkumpul dalam lib sahaja, 5 perangkap direkod). Peta v2.5 (3 refresh, backup vault V2.3/V2.4/V2.5). Baseline RM 63,912.00 (369 order) IDENTIK merentas semua 6 commit, setiap gate disahkan sesi utama sendiri sebelum commit. Insiden kecil push: token gh tiada scope workflow (kali pertama push fail CI), diselesaikan gh auth refresh interaktif owner; nota kosmetik CI: amaran deprecation Node 20 pada actions v4, boleh bump bila bila. Sebelum tu (31 Jul lewat malam, ROUND 3-5 SEMUA LIVE PROD, commit f06ea82 + 5be3037, deploy Ready) Sambungan hari sama, 3 round lagi fan-out subagent. ROUND 3: (a) BILL_ID J&T IKUT KANDUNGAN, penemuan bisnes penting: statement J&T rupanya HARIAN bukan bulanan (PDF = COD Statement rasmi dengan invois harian, XLS export = BANYAK bil harian dalam satu fail); id baru `akaun-YYYYMMDD` per baris (jnt_bill_id), XLS dan PDF hari sama keluar id SAMA, kuarantin double-bill hidup semula (mutation check bukti ia mati bawah derivasi lama), fallback jujur `akaun-UNDATED` + batang nama fail bila tiada token JTMY; skrip backfillJntBillId.py siap (dry-run default, guard duit SystemExit kalau jumlah berubah, bank_deposits sengaja tak disentuh); KEPUTUSAN OWNER: prod ikut pilihan (b) padam data J&T + team re-upload nama fail ASAL (fail live dah di-rename jadi id akan hodoh kalau backfill), rujukan bank_deposits J&T lama kena taip semula finance. (b) VOUCH DELETE wallet/prepaid: 2 jadual additive wallet_uploads + prepaid_uploads cermin corak order_uploads, 3 kelas baris (eksklusif padam, kongsi kekal + re-point, LEGACY kekal + dilapor, sengaja bukan fallback padam sebab prod 100% legacy), uploadVouchSchema.ts, ujian +7 Python +34 TS assertion. (c) LABEL "Amount unreadable": komponen AmountCell + 10 tempat UI, tiada lagi RM 0.00 palsu (search page ada paksaan ?? 0 literal), ResolveModal tak pra-isi adjustment hantu. (d) testGifts DIPULIHKAN: seed self-contained (bukan ubah snapshot, testMutations pin 9 SKU), FAIL lama tutup, gelung byGiftType yang tak pernah jalan kini bergigi, DAFTAR dalam suite = npm test 19 langkah. ROUND 4: (a) UI Uploads papar medan kept baru (wallet/prepaid shared/legacy) di mesej delete + log audit, senyap bila sifar. (b) nota kaki integRisk di stream courier + CHIP bila ada baris unreadable ("real exposure is higher", isyarat sama AmountCell, hormat penapis tarikh, sifar sentuhan enjin). (c) SNAPSHOT RESOLUTION FAIL-CLOSED: amount tak diketahui disimpan NULL bermakna (bukan RM0 palsu), pelarasan bernombor DITOLAK di pintu propose (GUARD 3b), kelulusan NAIK ADMIN (GUARD 2b, peer 403), cap jari guna token "unreadable"; kes prod lama bernombor TAK terusik langsung, kes lama yang pernah lulus atas RM0 palsu akan bertanda "Reopened, figures changed" (sengaja, jujur); 6 ujian baru bahagian F, 4 mutation menggigit. (d) peta v2.1 (2 kabinet vouch, bill_id harian, 237 ujian, driftCheck kini baca CREATE TABLE dari webApp/lib juga, backup vault V2.1). ROUND 5: (a) docs/enjinReconDivergen.md tambah D8 (tarikh songsang + cascade, DIBAIKI) + D9 (sentinel whitespace, DIDOKUMEN) + pembetulan fakta basi lain (D1 sebenarnya dah dibaiki, 12 semakan TS dikira dari kod); doktrinFleet.md dikemaskini (gate 19 langkah, parser 225, baki peta SELESAI, baki terbuka = F31 sahaja tunggu sampel refund CHIP, nota eksplisit "guard bertambah baik = risiko turun bukan kebenaran naik"). (b) AUDIT EXPORT: tiada RM0.00 bohong TAPI 2 masalah sebenar dibaiki, Close Pack TERKURANG KIRA SENYAP (SUM SQL langkau NULL, disahkan dev PG) kini ada kolum "Unreadable lines" per period + panel amaran, dan 5 laluan CSV dapat token UNREADABLE (teks, Excel SUM abaikan, takrif sekali CSV_UNREADABLE di ExportCsv.tsx); ekor sengaja ditinggal: kolum bil CSV per-stream (perlu kiraan scoped dari enjin) + net_remit CSV stokis (samar tanpa enjin). (c) ujian fx_adjustment atas AMOUNT_MISMATCH sintetik (kes Singapura RM12.50, alur penuh propose sampai settled, streamSummary deep-equal identik, fingerprint hidup, kes kawalan RM320 naik admin), pending lama fx TUTUP tanpa tunggu data sebenar; PENEMUAN DASAR direkod: ambang RM300 dinilai atas adjustAmount yang DITAIP bukan saiz beza baris, beza besar boleh ditutup peer selagi adjust kosong, ujian penanda "DASAR SEMASA" dipasang, keputusan owner menunggu. (d) panduanFinance.md dikemaskini (jadual 9 sebab tolakan, nota CHIP Status wajib, J&T harian + nama asal, mesej kept, Amount unreadable, Ninja turun) + DRAF PESANAN WHATSAPP siap-hantar untuk owner (salinan dalam nota vault dicciFinanceRound3Hingga5); fee unreadable search ditutup dua lapisan (skrin + CSV). Verify 3 KALI verifier bebas (round 3, 4, 5): baseline RM 63,912.00 (369 order) md5 52448276 identik SETIAP kali, parity 3 enjin LULUS, npm test 19/19, tsc + eslint bersih, enjin recon disahkan TIDAK tersentuh sepanjang round 3-5. Insiden kecil: satu deploy gagal sebab dijalankan dari root bukan webApp/ (alias prod tak terjejas, redeploy betul terus Ready). Sebelum tu (30-31 Jul, 2 ROUND HARDENING ENJIN + INGEST, SEMUA LIVE PROD) Sesi fan-out subagent: double check 3 zon dulu (enjin/repo/prod SEMUA hijau, penemuan: commit c58fcd7 rupanya dah push, Clerk prod masih pk_test instance dev). ROUND 1 (commit 65f2d81): (a) baris bil RM0 TAK lagi sahkan order "duit masuk" , syarat cod_amount>0 di keempat empat tempat (reconcile.py helper _duit_masuk, reconSql.py, recon.ts pemalar COD_LINE_OK dalam CONF_SQL+COD_LINE_EXISTS, db.confirmed_paid_order_ids); kesan live TERUKUR: 96 exception palsu duit_masuk_order_returned (RM17,271) hilang dari senarai siasatan, 135 order tak lagi tersalah confirmed (96 Returned COD + 30 CHIP + 8 Bank Transfer + 1 BT Returned; punca semua = baris caj Ninja Returned to Sender RM0), itu PEMBETULAN bukan data hilang; (b) mod runtuh tarikh ditutup: format="mixed" di reconcile.py (2 tempat) + reconSql umur_hari, verify sah satu sel tarikh bukan kanonik di kedudukan pertama boleh runtuhkan parsing SATU STREAM penuh dalam E1 (ujian: RM57,937 duit lewat lesap senyap), pintu ingest memang dah menapis jadi laten; (c) 2 divergen reconTrust terakhir DIVERIFY SAH (tarikhSongsang 14/24 perangkap + sentinelKosong tab/newline/NBSP sahaja, dua dua LATEN 0 baris data sebenar, E1 sentiasa yang terasing), fixture dinaikkan jadi ujian kekal self-contained, testReconEdgeCases 18->34 + TS 7->11, mutation check terbukti (fix ditanggal = 10 gagal); (d) guard nilai duit jadi sejagat guard_feed_values + registry FEED_MONEY untuk SEMUA 6 feed (dulu Fighter sahaja; primary=kosong+RM0, secondary=RM0 sahaja), parser 116->150 + hardening CHIP (Status jadi lajur WAJIB, strip Type, prefix FIGHTER- case-insensitive di hadapan sahaja, reason baru no_payment_rows amber bila 100% baris ditapis). ROUND 2 (commit 4b9fd82): jumlah kawalan DALAM fail disemak silang (guard_control_total: Ninja baris TOTAL kaki SOA, DHL PDF Total before deduction/Sum Total/Payment amount, J&T GRAND TOTAL dipindah ke guard sama; reason baru tally_mismatch; fail tanpa baris total dilangkau senyap), DHL Total deduction bukan sifar kini DITOLAK reason not_modelled (dulu fee dikunci 0.0 senyap = lapor duit lebih), J&T PDF semak per baris cod-fee≈net + fee baris reversal kekal NEGATIF (bukan buang abs() bulat, guna -(txn+SST) supaya sampel sebenar identik sen demi sen), duplikat kunci DALAM satu fail dikesan (identik=dedup jujur, beza=tolak reason duplicate_rows, BILLS_UPSERT dipindah selepas guard = fail tolak sifar kesan DB), wallet dibersihkan (tapis baris tanpa Transaction ID yang dulu runtuh jadi satu rekod "nan", strip teks seller/status/type/source), header PDF DHL kini dibaca ikut NAMA lajur (peta alias) bukan posisi tetap; parser 150->207, semua tolakan J&T PDF jadi IngestError berkod (dulu ValueError mentah = server error). UploadModal: tally_mismatch + duplicate_rows masuk HARD_REJECT merah, not_modelled SENGAJA amber (kerani tak boleh betulkan, kena naik owner). testGifts.ts: oracle kini IMPORT CONF_SQL terus dari lib/recon.ts (drift mustahil secara struktur, dulu salinan tangan dah drift 2 kali) + kes sintetik baris RM0 dengan kes kawalan RM50; PENEMUAN: 1 FAIL pre-existing (fixture dev: SKU order MYSE-* sifar pertindihan dengan sku_bottles, disahkan wujud di HEAD lama, bukan kod; testGifts TIADA dalam npm test jadi tak dipantau), keputusan owner menunggu: backfill fixture vs longgar pemilih + daftar dalam testAll. TRIAGE PENUH 42 finding duitAnchor yang mati dalam log: 3 dicover, 11 tolak (duplikat kes yang dah diadili), 1 tak cukup info (F31 refund CHIP, perlu sampel sebenar), 27 terbuka dikelompok 13 kerja ranked, lebih kurang 9 ditutup round 1+2; BAKI UTAMA: bill_id J&T = nombor akaun bukan nombor statement (DISAHKAN 2 konvensyen nama fail vendor, semua statement bulanan runtuh jadi JTMY031691, kuarantin double-bill mati + settlement_date bertimpa, perlu sesi sendiri sebab migrasi data sedia ada), vouch delete wallet/prepaid (padam fail boleh buang baris fail lain), label khas CHIP Amount NULL (sekarang dipapar mismatch RM0 = salah diagnosis). PELAN CLERK PRODUCTION siap (runbook clerkProductionRunbook.md gitignored ditampal 10 tempat, 245->378 baris): domain cadangan finance.dicci.com.my (dicci.com.my milik Dicci, DNS Cloudflare, contoh lama finance.diccigroup.com SALAH tak wujud), allowlist production WAJIB masuk diccifinancehq@gmail.com (kalau tercicir approval Resolution mati), MFA + buang "Secured by Clerk" = Clerk Pro USD25/bulan (instance production sendiri percuma), KRITIKAL: Clerk tak boleh pindah user antara instance jadi SEMUA team daftar semula via invitation dashboard (app tiada /sign-up, sengaja restricted), .vercel.app BERHENTI jadi pintu masuk lepas cutover; blocker tunggal = akses Cloudflare owner. Kebersihan: emel dalaman fixture ujian ke example.com (repo public, commit c7ab90a); amaran kekal dari audit: branch lokal backupSebelumTarikPeta ada peta/ dalam sejarah, JANGAN SESEKALI push branch tu. PETA v2.0 (bukan v1.10, agent ikut konvensyen versi peta sendiri yang guling minor pada 9): page baru D-12 Review, 2 kabinet recon_resolutions/recon_resolution_events, flow ke-9 Settle Mismatch, fakta enjin RM0/CHIP Status masuk nota kabinet, driftCheck.mjs kini baca CREATE TABLE dari webApp/lib/*.ts juga (jadual Resolution lahir di resolutionsSchema.ts), ujian peta 202->220, backup vault petaDicciFinanceV2.0.html. Gate penuh lulus 2 KALI oleh verifier bebas: npm test 18/18, parity 3 enjin LULUS (jnt 932 tally 369, dhl 33, ninja 53, chip 176), baseline RM 63,912.00 (369 order) md5 52448276aa50f9dd869816e1794c5a4e IDENTIK, tsc bersih, check:engine in sync. 2 deploy Vercel prod READY (terkini r1cki4n2k) + Streamlit auto-redeploy dari push. PESANAN TEAM belum sampai (owner kena sampaikan): (i) statement CHIP tanpa lajur Status kini ditolak masa upload, sengaja; (ii) angka exception Ninja akan turun mendadak = pembetulan bukan data hilang; (iii) fail bil kini disemak jumlah kawalan dalam fail, tak tally = merah dengan sebab. Sebelum tu (29 Jul, SIAP DEPLOY) CIRI BARU "Resolution layer" (settle mismatch/tak tally) hasil /timbang panel 5 lensa + judge silang + research pass sahkan 20 dakwaan kod. Prinsip mutlak: resolve tukar STATUS baris sahaja, TAK PERNAH tukar DUIT, tally/variance/baldi kekal identik (diuji deep-equal streamSummary sebelum/selepas). Unit = satu baris exception, kunci subjek order_id (sisi order) atau awb (sisi duit), stream+kategori SENGAJA TAK masuk kunci (dua dua boleh berubah sendiri, disahkan research pass). Fingerprint dikira masa BACA, data berubah = kes stale serta merta, baris balik penuh ke baldi. Maker checker dikuatkuasa di SQL (bukan JS), ambang admin RM300 + had bulk peer 50 baris (keputusan owner), admin = Dicci Finance HQ (diccifinancehq@gmail.com) BUKAN owner (owner sengaja keluar dari gelung operasi). 9 reason code: processing_fee/courier_adjustment/partial_or_refund/data_entry_error (settle terus), fx_adjustment (BARU, settle terus, punca turun naik kadar tukaran mata wang order Singapura, selling_price RM tetap tapi duit masuk kurang, LIHAT nota bisnes bawah), bill not in yet (nama lama "Awaiting bill upload", snooze 30 hari, PENTING untuk baldi hilang_lewat 1,099 baris/73% kes yang sebenarnya artifak bil belum upload bukan bocor), pending_recovery + missing_parcel (BARU, snooze, parcel hilang tunggu tuntutan kurier) snooze 30 hari, written_off + other (admin sahaja). 2 jadual baru (recon_resolutions + recon_resolution_events append-only), API /api/resolutions (POST propose/PATCH decide/DELETE withdraw), page /impact/review (3 tab: Open bulk work, Awaiting approval split yours/ready-for-you, Settled+snoozed rollup), badge sidebar, butang Resolve inline di 3 tempat (page stream Integrity exceptions, page Not collected Overdue+Ghost, DAN drill parcel dalam BillsTable atas permintaan owner lepas test localhost). Enjin recon (recon.ts/reconcile.py/reconSql.py) diff SIFAR sepanjang seluruh kerja, disahkan berulang kali. BONUS 2 ciri kecil ikut permintaan owner masa test: (a) fix bug ingest Ninja (fail "balance" bukan SOA sebenar tersadung jadi server error 500, kini ditolak sopan dengan sebab jelas missing_columns, guard FEED_SCHEMA diluaskan ke SEMUA parser jnt/dhl/ninja/chip/wallet, ujian parser 102->116; SIASAT + TOLAK cadangan serap fail balance jadi bil, sebab goods_amount = selling_price tepat, akan cipta duit hantu RM17,874 + lebih kira net remit RM3,890 sebab fee jadi 0 senyap, keputusan: JANGAN serap, minta team hantar SOA sebenar); (b) penapis julat tarikh Order date di page Not collected (lib/uncollectedRange.ts, corak sama streamRange.ts, Ghost money SENGAJA tak ditapis sebab tiada order_date, dipapar jujur dengan nota, bug tersembunyi cap 5000 baris dielak dengan kira semula dari tmp_m penuh sebelum cap). Verify akhir menyeluruh (semua 32 fail gabungan sekali): tsc bersih, lint 0 error, npm test 18/18 SEMUA LULUS, baseline RM 63,912.00 byte-identik, diff enjin 0. RESET DATA PROD kali kedua sebelum ni (28 Jul petang) atas request team finance (arahan owner, tanpa backup diminta tapi cermin lokal tetap diambil: syncFromNeon + salinan data/backupSebelumReset20260728.db, 7,012 order/233 bil/48 SKU sebelum padam). Padam terus SQL ke Neon (route admin ada kill-switch ALLOW_STORE_RESET tak set di prod): SEMUA 13 jadual data dikosongkan TERMASUK sku_bottles + sku_gifts (beza dengan resetStore yang kekalkan SKU, ikut arahan owner "termasuk SKU semua"), app_meta dikecualikan (jejak boot, bukan data upload), satu transaksi, verify kiraan 0. PERISTIWA: team finance aktif upload MASA reset berjalan (kiraan naik antara dua run), dan mula re-upload serta merta lepas reset (41 bil DHL masuk dalam masa 2 minit), disahkan fresh ikut ingested_at, TIDAK dipadam. Nota: fail yang team upload beberapa minit SEBELUM reset turut terpadam, team perlu semak page Uploads dan upload semula dari awal; mapping botol SKU + gift config pun kosong, kena isi semula (auto-daftar SKU jalan masa upload Orders, nilai mapping kena set manual). Cache recon tak perlu diurus, upload team sendiri trigger revalidateTag. Sebelum tu (28 Jul, DALAM REPO) 3 syarat tambahan Jarvis untuk mod tangan DITUTUP hari sama nota diterima: (1) harness parity 3 enjin dipindah dari scratchpad /tmp masuk repo `parityHarness/` (8 skrip + wrapper jalan.sh + README BM; fail data fixture/dump ke parityHarness/data/ GITIGNORED sebab repo public, .gitignore tambah blok eksplisit; PENEMUAN: buatMirror.mjs scratchpad dah REPUT lawan HEAD sebab recon.ts kini export buildTmpM/buildTmpMPrepaid sendiri sejak fix 27 Jul, dibaiki jadi re-export terpilih; verify hujung ke hujung PARITY LULUS atas fixture.db DAN fixture2.db, dump reproduce identik s_*.json lama, banding.py terbukti exit 1 bila dump sengaja disilang, baseline RM 63,912.00 (369 order) kekal, md5 baselineRecon.db tak berubah; provenance fixture didokumen dalam README termasuk SQL delta fixture2 yang di-derive dari diff db), (2) doktrinFleet.md ditulis di root repo (gate ujian wajib npm test 15 langkah + check:engine + baseline, skop mod tangan = enjin recon + laluan Fighter sahaja, zon approval owner + zon haram mutlak, amaran ops npm test padam dev PG + RECON_TODAY=2026-06-18, peraturan berhenti-dan-tanya, baki peta tak lengkap), (3) skop mod tangan dikunci bertulis dalam doktrin sama. Verify bebas oleh sesi utama: jalan.sh dijalankan semula, LULUS exit 0. Mod tangan fleet kini BOLEH bergerak ikut doktrinFleet.md. Sebelum tu (27 Jul petang, SEMUA LIVE) Sesi housekeeping + 2 ciri UI ikut permintaan owner. (1) Proposal ca4b4a ditutup rejected/superseded (dicover testReconEdgeCases reconTrust), barisan proposal kosong. (2) Keputusan mini: guard ambang tarikh kosong DIPASANG di pintu ingest Fighter (Guard 5, F_EMPTY_RATIO_MAX 0.5 sama dengan guard duit, TestFighterDateGuard 6 -> 9, mutation check menggigit, baseline byte-identik md5 sama), kesan "N/A" TANGGUH dengan sebab. (3) Peta v1.8 (drift commit guard) + label "Bilik Server" kini derive MANIFEST.counts.cabinets + ujian VERSIONS derive dari MANIFEST.version, takkan stale lagi, 202 ujian, backup vault V1.8. (4) Fix scrollbar sidebar putih di Windows (color-scheme dark + thin scrollbar berjenama, globals.css sahaja). (5) CIRI BARU penapis julat tarikh: Dashboard /impact + 4 stream (jnt/dhl/ninja/chip), tapis atas order_date, SEMUA angka page ikut julat (KPI, carta, baldi, gift hero, Payment confirmation), lapisan read-only lib/dateRange.ts + streamRange.ts + dashboardRange.ts ATAS output recon (recon.ts diubah export sahaja, sifar logik), baris tanpa tarikh order kekal dipapar dengan nota jujur, state URL ?from=&to= boleh share, default All time = angka identik asal (diuji medan demi medan, suite testDateRange dalam npm test 15 langkah), bar penapis berjenama (pill emas aktif, rel emas kiri bila tapis aktif, ayat "Showing ...", a11y + reduced motion). Owner test visual sendiri di localhost, puas hati. Commit d72ad3f + deploy Vercel Ready. Peta v1.9 SIAP masa wrapSession 28 Jul (penapis tarikh masuk PAGES D-01/D-02, driftCheck selari, 202 ujian, backup vault V1.9); nota vault dicciFinancePenapisTarikh20260728 + memori projek dikemaskini. Sebelum tu (27 Jul pagi, FIX + DEPLOY) 3 divergen reconTrust DIBAIKI ikut keputusan owner (tarikh: normalize + guard di pintu ingest Fighter, reason suspect_values; luar skop: E2 SQL + E3 TS ikut E1 match_luar_skop bukan duit_hantu, angka Ghost money akan turun bila ada kes; pembundaran: reconcile.py _r2() ROUND_HALF_UP ganti round() half-even). BONUS penemuan: db.parse_dt lama boleh TUKAR BULAN-HARI ("2026-06-01" + dayfirst jadi 6 Jan) + jatuhkan sel senyap jadi NaT bila lajur campur format, dibaiki format="mixed", 0 beza atas data sebenar. Penggera kekal dipasang: testReconEdgeCases.py (E1 lawan E2 baris demi baris, 18 ujian) + testReconEdgeCases.ts (E3, 7 semakan) + TestFighterDateGuard (6 ujian), semua TERBUKTI gagal sebelum fix dan lulus selepas. Ujian parser 93 -> 100 (discover). Baseline RM 63,912.00 byte-identik (termasuk exceptions.csv), 0 baris data sebenar bertukar kategori, npm test 14/14 + check:engine + tsc lulus. Gap sedar didokumen: dialek sen SQLite vs Postgres pada digit ke-3 (100.005), pilih semantik Postgres, dikunci TestGapDialekSen, data sebenar sifar kes. docs/enjinReconDivergen.md tambah D5/D6/D7. Sebelum tu (27 Jul pagi, AUDIT SIAP, tiada kod diubah) reconTrust (giliran 2 graph engineering Jarvis, mod mata, 15 agent, run log agenticOsData/data/runs/1785116138883_reconTrust_82144a2f.json): parity 3 enjin atas baseline LULUS per-order + D2/D4 disahkan BETUL tertutup (fixture paksa kes). 10 kelas perangkap: 4 setuju semua, 6 divergen (3 DISAHKAN skeptik bebas + 2 belum diverify corak sama + 1 ditolak). TEMA: bila lari, E1 reconcile.py yang TERASING, E2 SQL + E3 TS sekata bertiga. 3 divergen sah: (1) order_date format bukan kanonik (tarikh sahaja/ISO-T/dd-mm) E1 belum_remit vs E2/E3 hilang_lewat; (2) AWB dikenali tapi order luar skop E1 match_luar_skop vs E2/E3 duit_hantu (page Ghost money terkesan); (3) pembundaran sen Python half-to-even vs SQLite/TS half-up (COD + prepaid). KEPUTUSAN OWNER DIPERLUKAN: divergen ni belum dibaiki (recon = zon kunci, perlu green light), dan soalan dasarnya "enjin mana jadi rujukan kebenaran untuk setiap kes". Fixture + skrip repro semua dalam scratchpad sesi (path dalam run log). Sebelum tu (27 Jul, DEPLOY LIVE) audit workflow duitAnchor (giliran 1 graph engineering Jarvis, mod mata, 15 agent, run log agenticOsData/data/runs/1785089364982_duitAnchor_7bbdf870.json) + amountGuard DILAKSANA: audit sahkan lubang RM0-senyap SAH tapi LATEN (sifar duit sebenar rosak, anchor fail-lawan-DB tally sempurna RM0 selisih), 2 finding sah dua-duanya laluan Fighter (K1 sel harga kosong/teks jadi RM0 senyap reason=ok, potensi RM207k-580k satu fail; K5 lajur Sales Commission/Item Count/SKUs hilang diganti 0 senyap). Fix = guard di PINTU ingest Fighter sahaja (ingest.py, db.to_num/recon TAK disentuh): guard schema lajur wajib (reason baru missing_columns) + amountGuard sel duit berisi-tapi-jadi-0 + lajur Selling Price >50% kosong (reason baru suspect_values), tolakan masuk ingest_rejections PII-safe, UploadModal papar merah. Ujian parser 76 -> 93, mutation check terbukti menggigit (guard dimatikan = 11 ujian gagal), baseline RM 63,912.00 (369 order) kekal byte-identik, npm test + check:engine lulus. Keputusan owner (A) amountGuard SELESAI dengan bukti. Sebelum tu (25 Jul, DEPLOY LIVE) page baru "Not collected" (link sidebar Money in) hasil audit /timbang panel 5 lensa: silang SEMUA kurier untuk order belum collect (Tab 1: overdue hilang_lewat merah + awaiting belum_remit neutral, DIPISAH, awaiting=normal tak dijumlah) + Ghost money (Tab 2: duit_hantu + nota jujur "selalunya order belum upload, semak Uploads dulu"). Pagar integriti: RECON_TODAY + ambang SAMA macam stream (satu sumber kebenaran, tak lawan dashboard), banner aging relatif bil diupload, cap "as of", tiada KPI gabungan. Fungsi recon read-only (uncollectedCourier/ghostPrepaid) guna buildTmpM SAMA, parity utuh. Ditolak (over-eng): carta/heatmap/KPI gabungan. Read-only, tiada schema baru. Sebelum tu SAMA HARI (24-25 Jul, LIVE): reset data prod (backup lokal data/backupSebelumReset20260724.db) + resetStore diluaskan padam jadual webApp + InfoTip (?) seluruh UI + upload PDF DHL & J&T (pdfplumber, tally-guard J&T) + allowlist Clerk tambah diccifinancehq@gmail.com + audit /timbang 6 item hardening ingestion: decode DHL toleran (fail terpotong 1 byte kini lulus), sebab berkod + mesej upload jujur English plain (ganti error mentah), kenal laporan status DHL bukan-bil, IngestResult kaya, log tolakan PII-safe (jadual ingest_rejections, fingerprint tanpa nilai baris), auto-sync enjin (syncEngine.mjs prebuild + gate check:engine, syncEngine.sh dibuang). Recon TAK disentuh, baseline RM 63,912.00 kekal, 76 ujian parser lulus. Sebelum tu (23 Jul, SEMUA LIVE): 4 divergen 3 enjin recon DITUTUP (TODAY dinamik zon KL per-request, guard shared AWB, recon prepaid CHIP + page /impact/streams/chip LIVE, sentinel NONE), sidebar 5 kumpulan + company switcher Dicci, pemburuan bug 10/14 dibaiki (delete selamat order_uploads, kuarantin bill_line_conflicts + Needs attention, variance Close Pack, log Price change), npm test + check:engine + 47 ujian parser, panduan finance lengkap, barisan proposal bersih, peta v1.6, graf UA segar 377 node. 26 commit PUSHED + 3 deploy Vercel READY malam sama. Laporan Jarvis: inbox laporanDicciFinance20260723. Sebelum tu: Fasa 1 pada dasarnya siap. WebApp Next.js LIVE di Vercel (diccigroupfinance.vercel.app), terkunci belakang Clerk auth (allowlist), cover 100% view Streamlit lama. Ciri terkini (22 Jul, LIVE): tapis client di Stockists + Uploads + tapis order dalam StockistModal (TableFilter kongsi; pintu carian Dashboard + link sidebar "Find order" DIBUANG atas arahan owner lepas test, page /impact/search kekal via URL terus), dan medan "Actual deposit date" dalam Bank Confirm + kolum Deposited dengan lag (+Nd) + masuk export (team TAK perlu adjust tarikh PDF J&T lagi). 3 feed disahkan dengan FAIL SEBENAR pertama kali (22 Jul, data/sampel/ gitignored): CHIP (112 paid masuk, 12 overdue ditolak, tekaan PREPAID_SUCCESS_STATUS SAH), DHL (UTF-16 .xls tepat jangkaan), Ninja (4 baris, nota: SOA campur baris caj bukan COD, akan nampak "pelik" dalam recon, bukan bug). Finance dah upload data sebenar (561 order, 2 bil J&T); 16 SKU live perlu masuk katalog di prod.
...
- SAMBUNG SESI BARU (kemaskini 2 Ogos: push 6 commit SELESAI + CI pertama hijau. Kerja kod menunggu KEPUTUSAN owner: (a) buka tangga 1 sistem migration, perlu pinda doktrinFleet dulu sebab schema/migrasi = zon haram mutlak, cadangan lalu Jarvis macam mod tangan dulu; (b) bebaskan testGifts dalam CI dengan perkaya fixture emas order_skus + sku_gifts, kecil; (c) SIAP 2 Ogos: actions bump ke v7 + Node CI 20 ke 24 selaras Vercel, dua dua run CI hijau. Tindakan manusia di bawah KEKAL belum buat: padam J&T + re-upload nama asal, re-upload wallet/CHIP isi vouch, bil Ninja + SOA sebenar, test Resolve akaun Dicci Finance HQ, WhatsApp team.) (kemaskini 31 Jul lewat malam: ROUND 3-5 LIVE DI PROD, lihat progress di atas; item lama bill_id/testGifts/fx SEMUA SELESAI). Keutamaan pending terkini, SEMUA tindakan manusia bukan kod: (1) OWNER: padam data J&T di prod (page Uploads) LEPAS TU hantar draf pesanan WhatsApp ke team (salinan penuh dalam nota vault dicciFinanceRound3Hingga5Kejujuran20260731) dan bagitahu "dah clear boleh re-upload"; (2) OWNER: test alur Resolve di prod dengan akaun Dicci Finance HQ, satu satunya benda tak boleh disahkan teknikal (ADMIN_EMAILS tak boleh dibaca balik), kini lebih penting sebab laluan admin fail-closed bergantung padanya; (3) TEAM: re-upload J&T nama fail ASAL + taip semula rujukan bank_deposits J&T + bil Ninja terkini + SOA Ninja sebenar; (4) KEPUTUSAN owner menunggu: dasar ambang RM300 (adjustAmount ditaip vs saiz beza baris, lihat progress round 5c), 2 ekor export (kolum bil CSV per-stream scoped + net_remit CSV stokis), kes lama "Reopened, figures changed" = jangkaan; (5) Clerk production tunggu akses Cloudflare owner (runbook lengkap); (6) kerja kod seterusnya bila sesuai: tangga 1 sistem migration (keputusan langkau turutan tangga diperlukan), F31 refund CHIP (tunggu sampel). Konteks 29 Jul di bawah: Commit c58fcd7 (32 fail) DAH PUSH ke GitHub, disahkan 30 Jul: c58fcd7 dan cfd6db9 dua dua ada dalam origin/main, git lokal selari penuh dengan remote. ADMIN_EMAILS Vercel prod ditukar ke diccifinancehq@gmail.com (Dicci Finance HQ jadi finance lead, sebelum ni KOSONG jadi semua fungsi admin mati di prod; nota jujur: Vercel tandakan env ni "Sensitive", nilai TAK BOLEH dibaca balik untuk sahkan selepas set, ditulis teliti dua kali tapi tiada jaminan teknikal 100% tanpa test sesi sebenar). Deploy Vercel prod dpl_8b4J2jU4Ho1s5jChVaFtkJx61oWp, READY, disahkan sihat (header x-clerk-auth-reason protect-rewrite macam biasa, bukan crash). Keutamaan pending: (1) owner/team finance test live di prod, terutama alur Resolve + kelulusan Dicci Finance HQ; (2) SELESAI 30 Jul: push ke GitHub dah siap, origin/main selari; (3) giliran 1+2 graph engineering SIAP (duitAnchor + reconTrust + fix); (4) tindakan owner: team upload bil J&T/Ninja/DHL terkini, minta SOA Ninja sebenar (bukan fail balance); (5) baki peta: 42 finding duitAnchor dalam log + 2 divergen reconTrust belum diverify (lihat doktrinFleet.md seksyen 7); (6) reason code fx_adjustment baru diuji atas kategori belum_remit sahaja (data dev tiada amount_mismatch Singapura sebenar), sahkan kelakuan bila kes sebenar muncul. 2 keputusan mini SELESAI 27 Jul petang: (a) guard ambang tarikh kosong DIPASANG (lihat seksyen 27 Jul lanjutan), (b) kesan "N/A" TANGGUH dengan sebab. Proposal ca4b4a DITUTUP 27 Jul (rejected, superseded: dicover sepenuhnya oleh testReconEdgeCases sesi reconTrust), barisan proposal projek ni kosong. Sejarah item asal 25 Jul di bawah:
  - (A) SELESAI 27 Jul: "amountGuard" dilaksana lepas audit duitAnchor ukur lubang (laten, sifar duit sebenar rosak). Guard di pintu ingest Fighter, db.to_num/recon tak disentuh, parity kekal. Lihat seksyen "Sesi 27 Jul".
  - (B) PENEMUAN 25 Jul (bukan bug, tindakan team): net remit prod sekarang RM 156,128 datang dari DHL SAHAJA (34 bil, settlement 2 Jan-3 Feb 2026), fee RM 0 (DHL advice takde fee). J&T + Ninja TIADA bil diupload; order ada 9,589 (Dis 2025-23 Jul). Jadi "overdue" di page Not collected majoriti ARTIFAK bil belum upload, bukan bocor. Team kena upload bil J&T/Ninja/DHL-terkini supaya net remit + aging betul.
...
- nota bisnes (2026-07-28, owner citakan semula lepas sesi lalu terputus sebelum sempat rekod): order Singapura, selling_price disimpan tetap dalam RM (contoh RM426), tapi duit yang betul betul masuk sikit kurang, PUNCA turun naik kadar tukaran mata wang (currency exchange), BUKAN bocor atau kesilapan data. Ini sumber sah untuk baldi amount_mismatch pada order lintas sempadan. Keputusan: reason code baru "Selling price adjustment" (kelas cost, settle terus bila lulus, bukan snooze, ambang RM300 + bulk 50 sedia ada tetap terpakai automatik ikut saiz kes) ditambah dalam ciri Resolution supaya team boleh tandakan kes macam ni dengan sebab yang tepat, bukan "Processing fee" atau "Data entry error" yang silap maksud.
...
- nota Jarvis (2026-07-26 malam): GRAPH ENGINEERING sedia guna untuk projek ni, mod MATA sahaja
  (fleet subagent baca dan lapor, HARAM tulis kod, repo ni zon haram tulis sebab pegang duit
  dan hampir tiada jaring ujian). 3 kerja siasat sedia spec, panggil dalam sesi projek ni dengan
  perkataan "workflow": (1) duitAnchor, audit 7 parser feed x 6 kelas kerosakan nilai, anchor =
  jumlah fail mentah mesti sama dengan SUM DB, ini JAWAPAN kepada gate (A) amountGuard, ukur dulu
  besar lubang sebelum owner putus; (2) reconTrust, fixture sintetik sengaja pecahkan 10 kelas
  logik atas 3 enjin recon, sebab docs/enjinReconDivergen.md mengaku D2/D4 lulus parity palsu
  atas data dev, baseline RM 63,912.00 (369 order) WAJIB kekal byte identik; (3) proposal
  ca4b4a (kunci fix AWB kongsi + sentinel NONE dengan ujian) diluluskan Jarvis, SELESAI 27 Jul:
  ditutup rejected/superseded, fixture reconTrust (testReconEdgeCases) dah cover kes sama. JALAN untuk buka mod TANGAN (fleet boleh tulis) nanti: pasang penggera
  (ujian dari kerja 1 dan 2), buktikan penggera berbunyi (sengaja rosakkan, ujian mesti merah),
  kunci baseline. Doktrin wajib: ~/.claude/doktrinOrkestrasi.md. Keputusan penuh + senarai
  giliran: knowledgeVault decisions/timbangGraphEngineeringPortfolio20260726.md.
- nota Jarvis (2026-07-28): MOD TANGAN LULUS BERSYARAT. Verifier bebas (Opus, context
  bersih) sahkan 3 syarat asal dipenuhi: penggera terbukti menggigit lewat mutation check
  bebas (enjin lama dipasang balik: 11/18 testReconEdgeCases.py gagal, guard dimatikan:
  11 ujian parser gagal, recon.ts lama: 2/7 gagal), baseline RM 63,912.00 (369 order)
  direproduksi tepat, parity 3 enjin lulus. TAPI fleet TULIS belum boleh bergerak sampai
  3 syarat tambahan ditutup (anggaran setengah hari): (1) pindahkan harness parity 3 enjin
  (banding.py, dumpPy.py, dumpTs.mts, fixture.db + dump e1/e2/e3) dari scratchpad /tmp
  masuk repo, ia aset paling penting dan sekarang lenyap bila Mac restart; (2) npm test
  jadi gate WAJIB alur fleet, tulis dalam doktrin fleet repo ni: haram commit tanpa suite
  hijau penuh dengan dev PG 5433 hidup (repo tiada CI, build Vercel tak jalankan ujian,
  jadi penggera hanya berbunyi kalau dijalankan); (3) skop mod tangan = enjin recon +
  laluan Fighter SAHAJA, ingest wallet/jnt/dhl/ninja/chip masih TIADA guard pintu, sentuh
  ia kekal perlu approval owner. Nota ketepatan dari verifier: TestFighterDateGuard kini
  9 ujian (bukan 6), parser 102 (bukan 100), npm test 15 langkah (bukan 14), "byte-identik"
  tepat untuk baseline + trio Python sahaja (e3.json parity multiset, bukan byte). Amaran
...
- kemaskini: 2026-08-02

## Sesi 31 Jul malam + 2 Ogos (fleet foundation: enjin kanonik, parity dalam suite, CI pertama)

Sesi fan-out fleet mod tangan (doktrinFleet dipatuhi penuh: agent tak commit/push, sesi
utama sahkan setiap gate sendiri sebelum commit). 6 commit, semua dah push 2 Ogos,
run CI GitHub pertama dalam sejarah repo LULUS penuh.

- **Keputusan bentuk enjin (diratifikasi owner, AskUserQuestion)**: Pilihan 1 dari 3.
  recon.ts kanonik, reconcile.py oracle beku, reconSql.py beku mati dengan Streamlit.
  Detail + pilihan ditolak: knowledgeVault decisions/keputusanEnjinKanonikRecon20260731.md.
- **1ef7ba4** fasa 0+1+3: header peranan 3 enjin + docs/enjinReconDivergen.md seksyen
  keputusan, checkReconConstants.mjs dalam suite (terbukti menggigit), TestSharedAwbKeras
  + liputan shared-AWB E3 (sebelum ni E3 sifar kes shared-AWB) + whitespace kunci D9.
- **3f4e6c9** fasa 2: genReconConstants.mjs jana lib/reconConstants.ts dari db.py/
  reconcile.py/theme.py, recon.ts import (re-export kekalkan simbol hiliran), prebuild
  + guard byte dalam build, npm run build Vercel disahkan lulus.
- **441d4b7** parity dalam suite: langkah akhir testAll (dev PG parity_tapak sendiri,
  ~3 saat warm), fail-loud bila parityHarness/data tiada; fixture sen digit ke-3 sisi
  PG (100.005/99.995/0.005 dua arah) + TestTarikhTepiKalendar (31/30, 29 Feb sah,
  29 Feb tak sah kekal gap D7, NULL aging, anti-cascade D8).
- **ee6e51b** CI pertama: ci.yml (Python 3.12 + Node 20 + PG service schema-only dari
  db.init_db) + testCi.mjs subset sintetik. **0ef16c9** golden + ratchet: fixture emas
  15 kategori (semua label KAT_LABEL_EN tercetus, assert liputan supaya fixture tak
  reput senyap), testGoldenParity E1 lwn E2; ratchet 203 titik (.query( lexical).
  **d746c33** CI 6 ke 15 langkah: seedGoldenPg bebaskan parityDump/parityCheck (E2 lwn
  E3), stockist, bank, dateRange, uncollected; tsc + ratchet masuk CI; docs/ciCoverage.md.
- **Bukti menggigit direkod setiap guard baru** (rosak sengaja = merah, pulih = hijau):
  konstan, parity suite (via reconSql), golden parity (via reconSql), parityCheck E3
  (via recon.ts operator), ratchet (3 arah), CI runner (via assertion edge TS).
- **Inventori company_id** (baca sahaja, Explore Opus): vault notes/
  inventoriCompanyIdDicciFinance20260731.md, 16 tenant + 2 global, PK komposit yang
  perlu, ON CONFLICT 10+4, semua query dalam webApp/lib sahaja (kelebihan besar untuk
  wrapper withCompany tangga 4), 5 perangkap (awb PK global, index unik resolutions,
  SKU join silang syarikat, vouch delete tanpa skop, dua salinan enjin Python).
- **Run log**: agenticOsData/data/runs/ 3 fail (fleetPenyatuanEnjinRecon,
  fleetFoundation6Kerja, fleetCiHardeningRatchet).

## Sesi 27 Jul pagi lanjutan (fix 3 divergen + insiden deploy + peta v1.7, SEMUA LIVE)

Sambungan terus audit reconTrust: owner putuskan arah 3 divergen (AskUserQuestion,
semua ikut recommended) dan fix dilaksana + deploy hari sama. Detail keputusan:
knowledgeVault decisions/keputusanDivergenRecon20260727.md.

- **Fix A pembundaran**: reconcile.py `_r2()` ROUND_HALF_UP (Decimal) ganti round()
  half-even, 2 titik (COD + prepaid). 0 baris data sebenar bertukar kategori.
- **Fix B luar skop**: reconSql.py (`not_sentinel_literal` pada known_trk dua dialek)
  + recon.ts (`NOT_SENTINEL_LITERAL` pada EXISTS match_luar_skop), ikut E1: bil AWB
  dikenali tapi order luar skop = match_luar_skop benign, BUKAN duit_hantu.
- **Fix C tarikh**: db.parse_dt `format="mixed"` (bonus: baiki bug TUKAR BULAN-HARI
  lama + sel jatuh senyap jadi NaT) + guard_fighter_dates di pintu ingest (tarikh
  tak boleh parse = tolak, reason suspect_values), F_DATE masuk lajur wajib.
- **Penggera kekal**: testReconEdgeCases.py (18, E1 lawan E2 row-by-row, fixture
  tempdir) + testReconEdgeCases.ts (7, E3 atas dev PG) + TestFighterDateGuard (6),
  masuk pipeline npm test (14 langkah). SEMUA terbukti gagal-sebelum lulus-selepas.
  Gap sedar didokumen: dialek sen SQLite vs Postgres digit ke-3 (TestGapDialekSen,
  pilih semantik Postgres, data sebenar sifar kes). docs/enjinReconDivergen.md
  tambah D5/D6/D7.
- **Insiden deploy (selesai, fix kekal)**: deploy pertama gagal "bundle 320MB > had
  225MB function Python". Punca: devPgData 266MB (digemukkan 16 db sisa agent
  reconTrust) masuk bundle sebab builder python TAK baca .gitignore. Fix:
  webApp/.vercelignore baru (senarai devPgData dll) + pin pandas<3 dalam
  requirements.txt (elak resolusi baru tarik pandas 3 + pyarrow). Deploy ketiga
  Ready, app disahkan sihat (404 pada curl = protect-rewrite Clerk untuk klien
  bukan-browser, perangai normal; browser sebenar dapat 307 handshake). Produksi
...
- **Keputusan mini SELESAI (27 Jul petang, keputusan owner)**: (a) DIPASANG, guard
  ambang tarikh kosong di guard_fighter_dates (Guard 5, guna F_EMPTY_RATIO_MAX 0.5
  sama dengan guard duit, lebih separuh Date kosong = tolak reason suspect_values),
  ujian TestFighterDateGuard 6 -> 9, mutation check terbukti menggigit (guard mati =
  2 ujian gagal), baseline byte-identik (md5 sama), npm test 14/14 + check:engine
  lulus. (b) TANGGUH dengan sebab: kesan "N/A" perlu keep_default_na=False sentuh
  SEMUA feed (audit semula 5 parser), sifar kes data sebenar, dan kes teruk (fail
  penuh N/A pada tarikh) kini ditangkap guard (a) sebab semua jadi kosong. Revisit
  bila penyatuan enjin tangga 2 atau insiden sebenar.

## Sesi 27 Jul pagi (audit reconTrust, TIADA kod diubah, keputusan owner diperlukan)

Giliran 2 graph engineering Jarvis: workflow reconTrust (mod MATA, 15 agent Opus:
1 jurutera tapak + 10 pemasang perangkap + 4 skeptik konteks bersih, ~102 minit,
1.61M token). Fixture sintetik sengaja pecahkan 10 kelas logik atas 3 enjin recon
(E1 reconcile.py, E2 reconSql.py dua dialek, E3 webApp/lib/recon.ts). Run log:
agenticOsData/data/runs/1785116138883_reconTrust_82144a2f.json (detail penuh +
path fixture/repro semua di dalamnya).

- **Berita baik dulu**: parity 3 enjin atas baseline LULUS per-order (multiset
  order_id|awb|bill_id|kategori identik, bukan agregat sahaja). D2 (guard AWB
  kongsi) + D4 (sentinel NONE) disahkan BETUL tertutup dengan fixture yang memaksa
  kes tersebut, bukan lulus palsu macam kebimbangan docs/enjinReconDivergen.md.
  Kawalan negatif dilakukan (harness terbukti sensitif, bukan sentiasa "LULUS").
- **4 kelas setuju semua enjin**: codDanPrepaid, statusBercanggah, bilKosongRm0,
  bilBerpecah.
- **3 divergen DISAHKAN skeptik bebas** (skeptik bina fixture sendiri dari kosong,
  bukan pakai fixture pemasang perangkap):
  1. **orderTanpaBil / format tarikh**: order_date bukan kanonik ("2026-06-03"
     tarikh sahaja, "2026-06-02T10:00:00" ISO-T, "01/06/2026") E1 kata belum_remit,
     E2+E3 kata hilang_lewat. Punca: E1 kira hari selepas parse datetime, E2/E3
     banding RENTETAN lawan ambang "2026-06-03 00:00:00". Format kanonik 19 aksara
     tepat sempadan 14/15 hari SEMUA setuju, jadi isu format sahaja.
  2. **bilTanpaOrder / luar skop**: AWB dikenali tapi order di luar skop stream,
     E1 kata match_luar_skop (benign), E2+E3 kata duit_hantu (alert bocor). Page
     "Ghost money" webApp bergantung terus pada kategori ni.
  3. **amountMismatch / pembundaran sen**: Python round() half-to-EVEN vs SQLite
     ROUND() half-up (TS ikut SQL). 8/19 perangkap sempadan sen divergen (COD +
     prepaid). Beza kasar (RM10, 1 sen penuh) semua setuju.
- **2 divergen belum diverify** (kuota verify 4): tarikhSongsang + sentinelKosong,
  corak SAMA (E1 keseorangan lawan E2+E3 sekata). 1 ditolak: awbKongsi (beza kod
  wujud tapi sifar divergen hidup).
- **TEMA BESAR untuk keputusan**: bila enjin lari, E1 Python (rujukan kebenaran
  diisytihar) yang TERASING, E2+E3 sekata. Soalan owner: untuk setiap divergen,
  enjin mana yang BETUL? (contoh: order tarikh pelik patut hilang_lewat ke
  belum_remit? bil luar skop patut benign ke alert?) Lepas keputusan, fix ikut
  PERATURAN ubah recon (reconcile.py dulu, parity harness, baru sync).
- **Sisa**: dev PG 5433 mungkin ada db recontrust_tapak + recontrust_syn (tak
  kritikal, buang bila senang). Tiada satu fail repo pun diubah, git bersih.

## Sesi 27 Jul (audit duitAnchor + amountGuard, DEPLOY Vercel LIVE)

Giliran 1 graph engineering Jarvis dijalankan: workflow duitAnchor (mod MATA, 15 agent
Opus: 1 juruukur anchor + 8 auditor laluan parser + 6 skeptik konteks bersih, doktrin
orkestrasi penuh, run log agenticOsData/data/runs/1785089364982_duitAnchor_7bbdf870.json).
Soalan dijawab: "duit rosak jadi RM0 senyap, besar mana lubang?" Jawapan: lubang SAH
wujud tapi LATEN, sifar duit sebenar rosak dalam data live.

- **Anchor tally sempurna**: satu-satunya fail sampel yang wujud dalam DB (DHL advice
  84728719) padan byte-perfect RM 3,162.00 / 16 baris / set AWB identik, selisih RM 0.
  Bonus: kembar .xls vs .pdf advice 84780324 keluarkan angka IDENTIK melalui 2 parser
  DHL berbeza. Selebihnya bukan lubang parser tapi bil belum upload (J&T RM 4,037,
  Ninja RM 587, CHIP RM 111,578, 2 advice DHL), sahkan penemuan (B) 25 Jul.
- **Statistik audit**: 53 finding mentah -> 48 unik -> 6 diverify -> 2 SAH, 4 ditolak
  skeptik (mekanisme benar, pencetus sintetik tak wujud dalam feed sebenar), 42 mati
  dalam log ikut kuota doktrin (senarai penuh dalam run log).
- **2 finding SAH (dua-dua laluan Fighter, laten)**: K1 sel harga kosong/teks
  ("PENDING", "-") jadi RM0 senyap reason=ok (fail 852 baris, SUM RM207,246.66 jadi
  RM0 tanpa amaran); K5 lajur Sales Commission/Item Count/SKUs hilang diganti 0/None
  senyap (ingest.py corak `else 0`), sedangkan Selling Price hilang lempar KeyError.
- **Fix amountGuard (implement + deploy)**: guard di PINTU ingest Fighter SAHAJA,
  db.to_num/reconcile/reconSql TAK disentuh. (1) guard_fighter_columns: 4 lajur wajib
  (Selling Price, Sales Commission, Item Count, SKUs), hilang = reason baru
  `missing_columns`; (2) guard_fighter_values: sel duit berisi-tapi-jadi-0 yang bukan
  sifar tulen = reason baru `suspect_values` (kiraan + maks 3 contoh), Selling Price
  >50% kosong = tolak (sel merged/export rosak); sel kosong tulen KEKAL dibenarkan
  (Sales Commission memang ada kosong sah dalam data sebenar). Tolakan masuk
  ingest_rejections PII-safe. UploadModal: set HARD_REJECT papar merah, mesej
  passthrough dari enjin. pyIngest/route.ts tak perlu ubah (passthrough disahkan).
- **Verifikasi**: ujian parser 76 -> 93, mutation check menggigit (guard dimatikan =
  6 failures + 5 errors), fail sampel sebenar tetap lulus (852 ok, SUM padan audit),
  baseline `RM 63,912.00 (369 order)` byte-identik, npm test 11 langkah + check:engine
  + tsc lulus.
- **Nota skop jujur**: lajur Fighter lain (Date, Status, dll) masih KeyError -> 500
  bila hilang (luar skop); "N/A" jatuh ke baldi kosong tulen (pandas default NA), nak
  kesan perlu keep_default_na=False yang sentuh SEMUA feed, tak dibuat tanpa green
  light; ambang 50% constant sengaja.
