...
os.environ.setdefault("DATABASE_URL", "postgresql://dev:dev@localhost:5433/dicci")
...
def dump_stream(s):
    """Bentuk agregat satu stream (courier atau prepaid) -> dict serasi parityCheck."""
    daily = [
...
def main():
    ...
    for key in ("jnt", "dhl", "ninja"):
        out[key] = dump_stream(reconSql.stream_summary(conn, "courier", key))
    # Prepaid (gateway CHIP): padan ikut order_id, bentuk agregat sama.
    for key in ("chip",):
        out[key] = dump_stream(reconSql.stream_summary(conn, "prepaid", key))
